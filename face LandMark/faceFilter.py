...
nose_img = cv2.imread('nose.png') # (860, 563) w,h ratio=563/860=0.65

prevTime = 0

bodyPart_landmark = [49,279,197,2,5] # 5 = center nose point
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            for lm_id, lm in enumerate(face_landmarks.landmark):
                ...
                if lm_id == bodyPart_landmark[0]:
                    leftnosex, lefttnosey = x, y
                if lm_id == bodyPart_landmark[1]:
                    rightnosex, rightnosey = x, y
                if lm_id == bodyPart_landmark[4]:
                    centernosex, centernosey = x, y

                # display only nose landmarks
                # if lm_id in bodyPart_landmark:
                #     cv2.putText(
                #         frame,
                #         str(lm_id),
                #         (x, y), 
